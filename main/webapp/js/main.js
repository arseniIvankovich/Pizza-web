...
size_select_elements.forEach(function (element) {
    var element_id = element.id.charAt(5);
    var price_name = "price-" + element_id;
    element.addEventListener("change", function () {
        var text = document.getElementById(price_name).textContent;
        var index = text.search(" ");
        if (element.options[0].value === "Big") {
            document.getElementById(price_name).textContent = (parseFloat(document.getElementById(price_name).textContent.substring(0, index)) + 5).toFixed(2) + " BYN";
        } else {
            document.getElementById(price_name).textContent = (parseFloat(document.getElementById(price_name).textContent.substring(0, index)) - 5).toFixed(2) + " BYN";
        }
    });
...
dough_select_elements.forEach(function (element) {
    var element_id = element.id.charAt(6);
    var price_name = "price-" + element_id;
    element.addEventListener("change", function () {
        var text = document.getElementById(price_name).textContent;
        var index = text.search(" ");
        if (element.options[0].value === "Thick") {
            document.getElementById(price_name).textContent = (parseFloat(document.getElementById(price_name).textContent.substring(0, index)) + 2).toFixed(2) + " BYN";
        } else {
            document.getElementById(price_name).textContent = (parseFloat(document.getElementById(price_name).textContent.substring(0, index)) - 2).toFixed(2) + " BYN";
        }
    });
...
var element1 = document.querySelectorAll(".menu-choices");
...
pizza_buskets.forEach(function (element) {
    element.addEventListener("click", function () {
        var element_id = element.id.charAt(7);
        var price_name = "price-" + element_id;
        var pizza_card = document.getElementById("pizza-" + element_id);
        var object = JSON.parse(localStorage["pizza"]);
        var flag = false;
        var ind = 0;
        var bool_size1 = true,
            bool_dough1 = true;
        ...
        for (var i = 0; i < object.length; i++) {
            if (object[i].name === pizza_card.querySelector(".pizza-card-title").textContent && object[i].size === bool_size1 && object[i].doughType === bool_dough1) {
                flag = true;
                ind = i;
            }
        }
        if (flag) object[ind].counter = object[ind].counter + 1;else {
            var pizza_index = pizza_card.querySelector(".busket-button-text").textContent.search(" ");
            var pizza_price = parseFloat(pizza_card.querySelector(".busket-button-text").textContent.substring(0, pizza_index));
            var addition = 0;
            var w = 0;
            var bool_size = true,
                bool_dough = true;
        ...
        var prev_price = parseFloat(busket.textContent.substring(0, busket_index));
        ...
        var new_quantity = parseInt(busket_quantity.textContent) + 1;
...
drink_buskets.forEach(function (element) {
    element.addEventListener("click", function () {
        var button_index = element.id.search("-");
        var element_id = element.id.substring(button_index + 1);
        var price_name = "dprice-" + element_id;

        var drink_card = document.getElementById("drink-" + element_id);
        var object = JSON.parse(localStorage["drinks"]);
        var flag = false;
        var ind = 0;
        for (var i = 0; i < object.length; i++) {
        ...
        if (flag) object[ind].counter = object[ind].counter + 1;else {
            var name_index = drink_card.querySelector(".drink-card-title").textContent.search(",");
            var drink_index = drink_card.querySelector(".drink-price-button-text").textContent.search(" ");
            var drink_price = parseFloat(drink_card.querySelector(".drink-price-button-text").textContent.substring(0, drink_index));
            var drink_item = {
                id: element_id,
